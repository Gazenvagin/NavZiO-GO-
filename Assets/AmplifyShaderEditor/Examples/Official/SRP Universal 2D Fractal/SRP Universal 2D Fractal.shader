	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv04 = IN.ase_texcoord1.xy * float2( 2,2 ) + float2( -1,-1 );
				float2 UV3 = ( ( pow( saferPower25 , (_SinTime.z*_ZoomScale + _ZoomOffset) ) * uv04 ) + _Center );
		...
		Pass
		{
			...
			half4 ShadowPassFragment(VertexOutput IN  ) : SV_TARGET
			{
				...
				float saferPower25 = max( _ZoomBase , 0.0001 );
				float2 uv04 = IN.ase_texcoord7.xy * float2( 2,2 ) + float2( -1,-1 );
				float2 UV3 = ( ( pow( saferPower25 , (_SinTime.z*_ZoomScale + _ZoomOffset) ) * uv04 ) + _Center );
				int MaxIter3 = _MaxIter;
				float Threshold3 = _Threshold;
				float localMandlebrot3 = Mandlebrot3( UV3 , MaxIter3 , Threshold3 );
...
/*ASEBEGIN
Version=17503
308;73;1253;1145;2062.434;1382.915;1.779731;True;False
...
ASEEND*/
//CHKSM=DF6CDBD2F3FE7A75B304628D04C95B3A3FB47B17