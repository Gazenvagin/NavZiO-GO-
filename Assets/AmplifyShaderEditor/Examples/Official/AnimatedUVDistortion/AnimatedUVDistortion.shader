    ...
    SubShader
    {
        ...
        Pass
        {
        	...
        	half4 frag (GraphVertexOutput IN ) : SV_Target
            {
				...
				float3 tex2DNode6 = UnpackNormalmapRGorAG( tex2D( _DistortTexture, uv_DistortTexture ), _UVDistortIntensity );
				...
				float4 temp_output_20_0 = ( _TintColor * ( tex2D( _MainTexture, ( float3( uv9 ,  0.0 ) + tex2DNode6 ).xy ) * tex2D( _MainTexture, ( float3( uv18 ,  0.0 ) + tex2DNode6 ).xy ) ) );
        ...
        Pass
        {
            ...
            half4 frag(VertexOutput IN) : SV_TARGET
            {
                UNITY_SETUP_INSTANCE_ID(IN);
           		...
           		float mulTime10 = _Time.y * _Speed;
           		float2 panner11 = ( mulTime10 * float2( -1,-1 ) + float2( 0,0 ));
           		float2 uv9 = IN.ase_texcoord.xy * float2( 1,1 ) + panner11;
           		float2 uv_DistortTexture = IN.ase_texcoord.xy * _DistortTexture_ST.xy + _DistortTexture_ST.zw;
           		float3 tex2DNode6 = UnpackNormalmapRGorAG( tex2D( _DistortTexture, uv_DistortTexture ), _UVDistortIntensity );
           		float mulTime15 = _Time.y * _Speed;
           		float2 panner17 = ( mulTime15 * float2( 1,0.5 ) + float2( 0,0 ));
           		float2 uv18 = IN.ase_texcoord.xy * float2( 1,1 ) + panner17;
           		float4 temp_output_20_0 = ( _TintColor * ( tex2D( _MainTexture, ( float3( uv9 ,  0.0 ) + tex2DNode6 ).xy ) * tex2D( _MainTexture, ( float3( uv18 ,  0.0 ) + tex2DNode6 ).xy ) ) );
...
/*ASEBEGIN
Version=16100
-1919;1;1918;1056;955.4291;554.2678;1;True;True
...
ASEEND*/
//CHKSM=CE968540555C42EF7615158D23E33970CC7D6E62