    ...
    SubShader
    {
        ...
        Pass
        {
        	...
        	half4 frag (GraphVertexOutput IN ) : SV_Target
            {
				...
				float3 lerpResult15 = lerp( UnpackNormalmapRGorAG( tex2D( _RockNormal, uv_RockNormal ), 1.0f ) , UnpackNormalmapRGorAG( tex2D( _SnowNormal, uv_SnowNormal ), 1.0f ) , saturate( ( WorldSpaceNormal.y * _SnowAmount ) ));
        ...
        Pass
        {
            ...
            half4 frag(VertexOutput IN) : SV_TARGET
            {
                UNITY_SETUP_INSTANCE_ID(IN);
           		...
           		float2 uv_RockAlbedo = IN.ase_texcoord.xy * _RockAlbedo_ST.xy + _RockAlbedo_ST.zw;
           		float2 uv_SnowAlbedo = IN.ase_texcoord.xy * _SnowAlbedo_ST.xy + _SnowAlbedo_ST.zw;
           		float2 uv_RockNormal = IN.ase_texcoord.xy * _RockNormal_ST.xy + _RockNormal_ST.zw;
           		float2 uv_SnowNormal = IN.ase_texcoord.xy * _SnowNormal_ST.xy + _SnowNormal_ST.zw;
           		float3 ase_worldNormal = IN.ase_texcoord1.xyz;
           		float3 lerpResult15 = lerp( UnpackNormalmapRGorAG( tex2D( _RockNormal, uv_RockNormal ), 1.0f ) , UnpackNormalmapRGorAG( tex2D( _SnowNormal, uv_SnowNormal ), 1.0f ) , saturate( ( ase_worldNormal.y * _SnowAmount ) ));
...
/*ASEBEGIN
Version=16100
-1919;1;1918;1056;1358.014;738.6148;1.255;True;True
...
WireConnection;23;9;17;0
ASEEND*/
//CHKSM=0FE509AAC27BD358076950AD88649AF33EF1C62B